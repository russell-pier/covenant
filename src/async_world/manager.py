...
class AsyncWorldManager:
    ...
    def __init__(self, world_config: WorldConfig):
        ...
        self.base_chunk_size = world_config.chunk_size  # Starting size (64 for lands_and_seas)
    ...
    def update_chunks(self, camera: Camera, screen_width: int = 80, screen_height: int = 50):
        ...
        # Early exit if no movement (optimization)
        if current_camera_chunk == self._last_camera_chunk:
            self._process_incoming_messages()  # Still process messages
            return  # Skip expensive chunk processing
